...
<p>One constraint shaped the whole project: no deep learning framework. NumPy for array maths, and nothing else. Not because frameworks are bad, but because <code class="inline">model.fit()</code> teaches you the API rather than the idea &mdash; and the parts a framework hides are exactly the parts worth understanding.</p>
<p>So the repository contains a working <code class="inline">im2col</code> convolution, its <code class="inline">col2im</code> gradient, max-pool forward and backward with index routing, dropout, softmax cross-entropy, and Adam. Every gradient was checked against numerical differentiation before training was allowed to start &mdash; a hand-written backward pass that is subtly wrong still trains, just badly, and that failure is almost impossible to diagnose after the fact.</p>
        </div>
        <br>
...
<p>Trained on the real MNIST set &mdash; 60,000 training and 10,000 test images, parsed by hand from the original IDX binary format rather than loaded from a helper library. Augmentation applies a random shift of up to two pixels and a rotation of up to twelve degrees on every sample.</p>
        </div>
        <br>
        <h3>03 Why a CNN, Not Just an MLP</h3>
        <div class="blog-content-body">
<p>The first working version was a plain multi-layer perceptron, 784&rarr;256&rarr;128&rarr;10, and it reached 98.64% on the test set. On typed and centred digits it looked finished. On real handwriting it was not &mdash; specifically, cursive nines with a long looping tail, which it read as a 3 or a 5 with confidence.</p>
<p>The reason is structural, not a matter of training longer. An MLP sees the image as 784 independent numbers with no notion that two pixels are adjacent, so a loop drawn slightly higher is, as far as the network is concerned, a completely different input. A convolution's filters slide, which means the same learned stroke detector fires wherever the stroke appears &mdash; and that translation tolerance is precisely what handwriting demands.</p>
...
<li>Paste into a 28&times;28 field, offset so the centre of mass lands dead centre &mdash; not the bounding box centre, which is a different point for an asymmetric digit like 1 or 7.</li>
</ul>
...
<ul>
<li><b>Per-pixel importance scan</b> &mdash; a saliency heatmap lights up brightest-first, showing which pixels moved the decision.</li>
<li><b>Input to Hidden 128</b> &mdash; signal lines flow into the first dense layer after the conv and pool stack, with active neurons lighting up.</li>
<li><b>Hidden 128 to Hidden 64</b> &mdash; the signal propagates deeper.</li>
<li><b>Output</b> &mdash; probability bars settle across the ten classes.</li>
</ul>
...
<p>Weights are committed, so <code class="inline">pip install -r requirements.txt</code> and <code class="inline">python app.py</code> is enough to run it &mdash; downloading MNIST is only needed to retrain or re-evaluate. The server is Waitress rather than Flask's development server, which is noticeably more stable on Windows, falling back to the dev server if Waitress is not installed.</p>
        </div>
        <br>
        <div class="d-flex">
