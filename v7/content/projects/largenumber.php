...
<p>A Java <code class="inline">long</code> stops at 9,223,372,036,854,775,807. Ask it for one more and it silently wraps around to a negative number. That ceiling is a property of the fixed 64 bits the type is given, not of arithmetic itself. The maths keeps working, the container runs out.</p>
...
<pre class="p-math-formula">[1] &lt;-&gt; [2] &lt;-&gt; [3] &lt;-&gt; [4]
 head              tail</pre>
<p>Doubly linked matters here. Column arithmetic runs right to left, from the least significant digit, so addition, subtraction and multiplication all start at <code class="inline">tail</code> and walk backwards through <code class="inline">prev</code>. Division runs the other way, left to right from <code class="inline">head</code>. A singly linked list would force one of those directions to be a reversal or a second pass.</p>
        </div>
        <br>
        <h3>02 The List</h3>
        <div class="blog-content-body">
...
<p>Nothing here converts back to a primitive at any point. A 500-digit number is never an <code class="inline">int</code>, not even briefly. The intermediate values in every algorithm below are lists too.</p>
        </div>
        <br>
...
for place in 1..20:
        ...
        while compare(remainder, b) &gt;= 0:
                ...
                d++
        emit decimal digit d</pre></div></div></div><div class="p-math-panel" data-panel="explained"><div class="p-math-explained"><p class="p-math-desc">There is no division primitive available here, so division is built out of the subtraction that already exists. The naive version, subtract the divisor one at a time and count, is correct but hopeless: dividing a 30-digit number by 7 would loop more times than there are atoms worth waiting for. The fix is to scale first. Before subtracting, the divisor is shifted left by appending zeros until one more zero would overshoot the remainder; that scaled divisor is then subtracted repeatedly, which can only happen between one and nine times, and contributes <code class="inline">digitCount &times; 10^zeroCount</code> to the quotient. Each outer pass produces one quotient digit and shrinks the remainder by an order of magnitude, so the work is proportional to the number of digits rather than to the value.</p><p class="p-math-desc">The decimal expansion reuses the same machinery. Appending a zero to the remainder is a multiplication by ten that costs one node, and each pass then yields exactly one decimal digit. It is capped at 20 places because a repeating expansion like <code class="inline">1 / 3</code> never terminates, and it breaks early when the remainder hits zero, so <code class="inline">10 / 4</code> prints <code class="inline">2.5</code> rather than <code class="inline">2.50000000000000000000</code>.</p></div></div><div class="p-math-panel" data-panel="example"><div class="p-math-example-wrap"><div class="p-math-example-scroll"><pre class="p-math-example">   a = 1234        b = 7
...
<li><b>Division by zero</b>: caught before the loop starts and reported as an error message, not an exception trace. An empty divisor would otherwise make the outer <code class="inline">while</code> spin forever, since the remainder never shrinks.</li>
